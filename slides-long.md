...
Note: Première partie : une expérience très personnelle de l'été dernier.
...
<div class="cols" style="align-items: center; margin-top: 0.35em; gap: 1.1em; padding: 0 0.6em">
<div style="flex: 0.4">
...
Note: Le jeu de foot. Noter la foule : des dizaines de milliers de supporters dans un seul InstancedMesh, un seul draw call.
...
<video src="assets/videos/neveu.mp4" data-autoplay muted controls style="max-height: 580px;"></video>

Note: Montrer le naturel de l'interaction.
...
> « On ne paie pas la fonctionnalité,
> on paie le **contexte** qu'elle oblige à parcourir. »

**Coût réel : 1 000 – 1 200 $** pour les deux jeux.
<br><span class="smaller">(et probablement ~50 $ dans un an, au rythme actuel)</span>

Note: ~99,8 % du coût part en lecture/relecture. C'est une loi économique du développement agentique : le contexte est la ressource rare.
...
Note: Le paradoxe fondateur de l'exposé : le langage disparaît de l'interface… et prolifère dans l'implémentation, jusqu'à 14 242 langages recensés (PL Catalog, 7 sources croisées ; on y revient en fin d'exposé). Poser la dernière question avec un brin d'ironie : c'est la question de l'exposé, version provocante.
...
Note: Deuxième partie : l'étude systématique.
...
Parce qu'on peut **mesurer objectivement**, quel que soit le langage :
...
Note: Troisième partie. Trois langages conçus pour être impossibles… ou pour rire.
...
Pas un jouet : un **vrai moteur UCI** :
...
Note: Langage absurde par construction, publié le 7 mars 2026 et attaqué le 12 : aucun exemple possible dans les données d'entraînement. La suite : que peut en faire un agent qui ne connaît QUE le dépôt ?
...
Brainfuck, M&M's, ArnoldC : trois langages **maîtrisés séparément** par l'agent.

Alors, le défi lancé, assez fou : **les empiler**, chacun interprétant le suivant.

<span class="big" style="font-size:1.6em">BF → M&M's → Schwarzenegger → JVM</span>

<p class="smallest">(Seule exception de l'exposé : ici, la démarche a été cadrée par l'humain, sans jamais toucher au code.)</p>

Note: La démarche compte : maîtrise séparée démontrée (un moteur d'échecs en Brainfuck, 26/26 en MnM, 25 défis en ArnoldC), donc on monte d'un cran. On va d'abord VOIR les programmes, puis le schéma d'ensemble. Tour de Babel fonctionnelle.
...
Note: Le langage-blague hérite de 30 ans d'ingénierie JVM. Et l'agent ne fait pas que consommer le langage : il RÉPARE sa toolchain. Contribution réelle à un projet open source.
...
Note: Quatrième partie : l'étude COBOL, avec un industriel dans la boucle.
...
COBOL = cas « **catastrophique** » pour l'IA générative :
...
## Que fait l'agent de ses journées ?

![Répartition des tâches](assets/figs/cobol-se_tasks-1.png) <!-- .element: style="max-height: 470px" -->

<span class="big">~85 % <small>du temps actif = compiler + lire/comprendre le code</small></span>

Note: build 43 % + understanding 42 %. L'agent vit dans une boucle compile-exécute-inspecte. Le « génie logiciel » de l'agent ressemble beaucoup au nôtre.
...
Note: Cinquième partie, courte : la synthèse transverse.
...
Note: Dernière grande partie : prospective, assumée comme telle, mais nourrie par les données de l'exposé. Chaque hypothèse est discutée avec un éclairage complémentaire (génie logiciel, publics, mathématiques formelles, encyclopédie).
...
Note: Reprise du panorama « Who Codes When AI Can Generate Code? » (séminaire SOTELO). Chaque contexte a ses exigences : le prototype jetable du dimanche et le système de paie n'ont pas les mêmes règles.
...
Le rêve : des agents qui **explorent, formalisent, prouvent**, et repoussent les mathématiques.
...
Note: On a 30 minutes de questions : laisser la slide Références affichée, touche O pour retrouver une slide. Questions fréquentes à anticiper : « et la sécurité ? », « quels métiers disparaissent ? », « pourquoi Claude et pas X ? », « combien ça coûte vraiment ? ».
